export default async function handler(req, res) {
  ...
  try {
    ...
    const media_url = data.video_url;
    const caption = data.img_caption || data.caption || "";

    if (!api_token || !mobile || !media_url) {
      return res.status(400).json({ status: false, error: "api_token, mobile, video_url required" });
    }
    ...
    const response = await fetch("https://whatsbot.tech/api/send_video?" + params.toString());
