export default async function handler(req, res) {
  ...
  try {
    ...
    const media_url = data.doc_url;
    const caption = data.img_caption || data.caption || "";

    if (!api_token || !mobile || !media_url) {
      return res.status(400).json({ status: false, error: "api_token, mobile, doc_url required" });
    }

    const params = new URLSearchParams();
    params.set("api_token", api_token);
    params.set("mobile", mobile);
    params.set("doc_url", media_url);
    
    if (device_id) params.set("device_id", device_id);

    const response = await fetch("https://whatsbot.tech/api/send_doc?" + params.toString());
